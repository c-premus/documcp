    ...
    <div class="grid grid-cols-1 gap-6 lg:grid-cols-3">
      ...
      <div class="lg:col-span-1">
        ...
        <div v-if="store.searchLoading" class="flex items-center justify-center py-8">
          <div class="h-6 w-6 animate-spin rounded-full border-4 border-gray-300 border-t-indigo-600" />
        </div>

        <!-- Search results -->
        <ul v-else-if="store.searchResults.length > 0" class="divide-y divide-gray-200 rounded-lg border border-gray-200 bg-white overflow-hidden">
          <li
          ...
          >
            <p class="text-sm font-medium text-indigo-600">{{ result.title }}</p>
            <p v-if="result.snippet" class="mt-1 text-xs text-gray-500 line-clamp-2" v-html="DOMPurify.sanitize(result.snippet)" />
          </li>
        </ul>

        <!-- Empty search state -->
        <div v-else-if="searchQuery.trim() !== '' && !store.searchLoading" class="text-center py-8 text-sm text-gray-500">
          No results found.
        </div>
      ...
      <div class="lg:col-span-2">
        ...
        <!-- Article content -->
        <div v-else-if="store.currentArticle !== null" class="rounded-lg border border-gray-200 bg-white p-6">
          <h2 class="text-xl font-bold text-gray-900 mb-4">{{ store.currentArticle.title }}</h2>
          ...
          <!-- Plain text content -->
          <pre v-else class="whitespace-pre-wrap text-sm text-gray-700 font-mono">{{ store.currentArticle.content }}</pre>
        </div>

        <!-- No article selected -->
        <div v-else class="flex flex-col items-center justify-center py-16 text-center rounded-lg border border-dashed border-gray-300 bg-gray-50">
          <p class="text-sm text-gray-500">Search and select an article to view its content.</p>
        </div>
      </div>
    </div>
  </div>
