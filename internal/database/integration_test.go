...
func TestRunMigrations_Success(t *testing.T) {
	...
	db := database.SQLDBFromPool(pool)
	defer db.Close()
	...
	_, err = pool.Exec(ctx, "SELECT 1 FROM documents LIMIT 0")
	assert.NoError(t, err)

	_, err = pool.Exec(ctx, "SELECT 1 FROM users LIMIT 0")
	assert.NoError(t, err)
}

func TestRunMigrations_InvalidDir(t *testing.T) {
	pool, err := database.NewPgxPool(context.Background(), testDSN, 5, 2, 30*time.Minute, 5*time.Minute)
	require.NoError(t, err)
	defer pool.Close()
	...
	err = database.RunMigrations(db, "/nonexistent/migrations")
	assert.Error(t, err)
}
...
func TestRunRiverMigrations_Success(t *testing.T) {
	...
	// Run goose migrations first (River depends on schema).
	db := database.SQLDBFromPool(pool)
	defer db.Close()
	...
	_, err = pool.Exec(ctx, "SELECT 1 FROM river_migration LIMIT 0")
	assert.NoError(t, err)
}
