...
type Document struct {
	...
	TagsText     sql.NullString  `db:"tags_text" json:"-"`
...
// ParseMetadata decodes the JSON metadata into the provided destination.
// Returns nil when metadata is absent (NULL or empty) — callers use the
// zero value of dest in that case.
func (d *Document) ParseMetadata(dest any) error {
	if len(d.Metadata) == 0 {
		return nil
	}
	if err := json.Unmarshal(d.Metadata, dest); err != nil {
...
func (dv *DocumentVersion) ParseMetadata(dest any) error {
	...
	if err := json.Unmarshal(dv.Metadata, dest); err != nil {
