...
func TestOAuthClientHandler_List(t *testing.T) {
	t.Parallel()

	h := NewOAuthClientHandler(nil, slog.New(slog.NewTextHandler(io.Discard, nil)))
...
func TestOAuthClientHandler_Show(t *testing.T) {
	t.Parallel()

	h := NewOAuthClientHandler(nil, slog.New(slog.NewTextHandler(io.Discard, nil)))
	req := httptest.NewRequest(http.MethodGet, "/api/admin/oauth-clients/client-1", nil)
	req = chiContext(req, map[string]string{"clientId": "client-1"})
	rr := httptest.NewRecorder()
